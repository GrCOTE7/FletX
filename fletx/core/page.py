...
import flet as ft
from typing import Union, List, Optional, Any, Dict, Type, TypeVar, Callable
...
class FletXPage(ft.Container, ABC):
    ...
    def build_floating_action_button_location(
        self,
    ) -> Optional[ft.FloatingActionButtonLocation]:
        """
    ...
    def build_bottom_sheet(self, *args, **extra_kwargs) -> ft.BottomSheet:
        """
        Builds bottom sheet.
    ...
    def build_navigation_widgets(self):
        ...
        self.view.floating_action_button_location = (
    ...
    def get_controller(
        self, controller_class: Type[T], tag: str = None, lazy: bool = True
    ) -> T:
        """Gets or creates a controller with automatic lifecycle management"""

        controller_key = (
            f"{controller_class.__name__}_{tag}" if tag else controller_class.__name__
        )
    ...
    def inject_controller(self, controller: FletXController, tag: str = None):
    ...
    def remove_controller(self, controller_class: Type[T], tag: str = None) -> bool:
    ...
    def watch(
        self, reactive_obj: Reactive, callback: Callable, immediate: bool = False
    ):
        ...
        if hasattr(reactive_obj, "listen"):
            ...
            self.add_cleanup(
                lambda: observer.dispose() if hasattr(observer, "dispose") else None
            )
            return observer
        else:
    ...
    def watch_multiple(
        self, reactive_objects: List[Any], callback: Callable, immediate: bool = False
    ):
    ...
    def remove_keyboard_shortcut(self, key_combination: str) -> bool:
        """Remove a keyboard shortcut"""

        self.logger.debug(f"{key_combination} shortcut handler removed.")
    ...
    def on_long_press(self, callback: Callable[[ft.LongPressStartEvent], None]):
        """Handle long press gestures"""

        if self._enable_gestures:
            self._gesture_handlers["long_press"] = callback

    def on_scale(self, callback: Callable[[ft.ScaleUpdateEvent], None]):
    ...
    def show_loader(
        ...
        dialog = ft.AlertDialog(
            content=content if content else ft.ProgressRing(), on_dismiss=on_dismiss
        )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "effect_count": (
                len(self._effects._effects) if hasattr(self._effects, "_effects") else 0
            ),
    ...
    def _setup_built_in_handlers(self):
        ...
        def resize(e: ft.ControlEvent):
            ...
            self.width = getattr(e, "width", self.width)
            self.height = getattr(e, "height", self.height)
            self.refresh()
    ...
    def _build_page(self):
        ...
        try:
            ...
            self.content = ft.Container(
                content=ft.Text(f"Error building page: {str(e)}"), padding=20
            )

    def _add_event_handler(self, event_name: str, callback: Callable):
        ...
        self.add_effect(lambda: self._connect_event_handler(event_name, callback), [])
