...
class FletXRouter:
    ...
    def initialize(
        ...
        if auto_navigate:
            # Navigate to initial route on the currently running loop.
            try:
                loop = asyncio.get_running_loop()
            except RuntimeError:
                loop = get_event_loop()

            loop.create_task(cls._instance.navigate(initial_route, replace=True))
    ...
    def _on_flet_route_change(self, e: ft.RouteChangeEvent):
        ...
        if self.state.navigation_mode in [NavigationMode.NATIVE, NavigationMode.HYBRID]:
            ...
            if self._is_programmatic_route_update:
                return

            # Ignore no-op changes.
            if e.route == self.state.current_route.path:
                return

            # External URL changes must trigger a full navigation/render.
            get_event_loop().create_task(self.navigate(e.route, replace=True))
    ...
    async def navigate(
        ...
        *,
        ...
        try:
            ...
            self.logger.debug(
                f"created Component: {component_instance.__class__.__name__}"
            )
            ...
            if self.state.navigation_mode in [
                NavigationMode.NATIVE,
                NavigationMode.HYBRID,
            ]:
                self._is_programmatic_route_update = True
                try:
    ...
    def go_back(self) -> bool:
        ...
        run_async(lambda: self.navigate(previous_route.path, replace=True))
        return True
    ...
    def go_forward(self) -> bool:
        ...
        # Use async task for navigation
        run_async(lambda: self.navigate(forward_route.path, replace=True))
        return True
    ...
    def add_global_middleware(self, middleware: RouteMiddleware):
    ...
    async def _check_activation_guards(
        self, route_info: RouteInfo, route_def
    ) -> NavigationResult:
    ...
    async def _run_before_middleware(
        self, from_route: RouteInfo, to_route: RouteInfo
    ...
    # @worker_task(priority = Priority.HIGH)
    async def _run_error_middleware(self, error: Exception, route_info: RouteInfo):
    ...
    # @worker_task(priority = Priority.HIGH)
    async def _create_component(self, route_def, route_info: RouteInfo):
        """Create and initialize the route component."""

        component_class = route_def.component

        if isinstance(component_class, type) and issubclass(component_class, FletXPage):
            # FletX page
